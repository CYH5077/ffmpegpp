...
TEST(TRANS_MUX, TEST_2) {
    ...
    EXPECT_EQ(ff::transMux(inputContext, inputFilename).getType(), ff::AV_ERROR_TYPE::SUCCESS);
}

TEST(TRANS_MUX, TEST_3) {
    std::string inputFilename  = Config::SAMPLE_MP4;
    std::string outputFilename = "./trans_mux_test_3.mkv";
    EXPECT_EQ(ff::transMux(inputFilename, outputFilename).getType(), ff::AV_ERROR_TYPE::SUCCESS);
}